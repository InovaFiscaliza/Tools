...
This guide is based on https://github.com/abraunegg/onedrive/blob/master/docs/podman.md with modifications to the use case of running data analysis scripts in a Python container.
...
podman volume create regulatron_conf
```
...
To find exatly where the volume is created in the host system you may use:
...
```bash 
find -name 'onedrive_conf'
```

#### 4.2 Prepare the 'data' volume

Create the 'data' volume mapped to the data directory on your local filesystem with the following command:

```bash
podman volume create --opt type=none --opt device=/mnt/repoSFI/OneDrive --opt o=bind onedrive_data
```
...
These containers offer a simple monitoring-mode service for the OneDrive Client for Linux and Python to run data analysis scripts.

Alpine distribution was selected to reduce the size of the container.

Please check the edge tag to ensure that the latest features and bug fixes are taken into consideration.

Additionally there are specific version release tags for each release. Refer to https://hub.docker.com/r/driveone/onedrive/tags and https://hub.docker.com/_/python/tags for any other Docker tags you may be interested in.

> [!NOTE]
> The indicated container image tags corresponds to the versions tested.


## OneDrive Image


To set up the OneDrive Docker container, use the following command:
...
To set up the Python Docker container, use the following command:

```sh
docker pull python:3.10.15-alpine3.20
```


### 5. First run of Docker container under podman and performing authorization

The 'onedrive' client within the container first needs to be authorized with your Microsoft account. This is achieved by initially running podman in interactive mode.
...
podman run -it --name onedrive --user "${ONEDRIVE_UID}:${ONEDRIVE_GID}" --userns=keep-id -v onedrive_conf:/onedrive/conf:U,Z -v onedrive_data:/onedrive/data:U,Z  driveone/onedrive:edge
```

> [!IMPORTANT]
> For onedrive business you will need to approve the app in Azure AD. This need to be done online by the administrator
>
...
podman run -it --name regulatron -v regulatron_conf:/regulatron/conf -v /mnt/reposfi/oneDrive:/mnt/oneDrive/ python:3.10.15-alpine3.20

podman rm regulatron
...
## Running the OneDrive Container


To run the OneDrive container, use the following command:

```sh
docker run -it --rm --name onedrive -v /path/to/config:/config -v /path/to/data:/data driveone/onedrive:edge-alpine
```
